...
onMounted(() => {
  let url = '/interface/getInterfaceInfoById';
  let params={apiId:""};
  ...
  axios.post(url,params)
    .then(function (response) {
      ...
      bodyValue.value = Object.entries(JSON.parse(JSON.parse(formValue.value.apiSample).requestBody)).map(([key, value]) => ({
        key, value
      }))
      kvValue.value = JSON.parse(JSON.parse(formValue.value.apiSample).requestHeader).map((item)=>{
        return{
          key:item.name,
          value:item.value
        }
